...
% @Joost: Misschien de uitleg over ROC vroeger al vermelden en met dat standpunt verder werken?
Exercising \textsf{Rocchio} for the \texttt{StandardAnalyzer} yields the results shown in figure \ref{fig:rocchio-standard}. Compared to figures \ref{fig:initBP}, \ref{fig:bool} and \ref{fig:tfidf}, we can see clearly that the AUC is increased, resulting in a better ROC curve. Unfortunately, for our use-case, the ROC (and AUC) are not the best measures to use, seeing as they only show the quality of the order compared to a random order. Ideally, we'd want as many relevant documents as possible in the top-k, hence optimizing the PR curve. Therefore, the \textsf{Rocchio} algorithm combined with the standard analyzer performs worse than when using our custom analyzer.
