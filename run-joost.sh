...
echo "Compiling..."
mkdir -vp .build
export CLASSPATH=$CLASSPATH:$(find ~/Documents/Overig/lucene-8.3.0/ -name "lucene*.jar" | paste -sd ":" -):packages/json-simple-1.1.1.jar
...
echo -e "  $(find ~/Documents/Overig/lucene-8.3.0/ -name "lucene*.jar" | paste -sd "+" - | sed 's/+/\
  /g')" >> ./manifest.mf
